...
class Station:
    ...
    def __init__(self, process_time: float, expon_scale: float, station_name: str, path_events: str):
        self.pt: float = process_time
        self.name = station_name
        self.path_events = path_events
        self.expon_scale = expon_scale
    ...
    def run_station(self, env: simpy.Environment):
        while True:
            ...
            yield env.timeout(self._apply_var(self.pt, self.expon_scale))
            aux.write_new_row(path=self.path_events, row=[
    ...
    def _apply_var(self, pt: float, s : float) -> float:
        # return max(0, random.gauss(pt, pt*STD_DEV))
        return max(0, expon.rvs(scale=pt*s, loc=pt, size=1)[0])
